...
export default class PhotoBrowser extends React.Component {
  ...
  constructor(props, context) {
    ...
    const { mediaList, startOnGrid, initialIndex, title } = props;
  ...
  componentWillReceiveProps(nextProps) {
    const mediaList = nextProps.mediaList.slice();
    this.setState({
      dataSource: this._createDataSource(mediaList),
      mediaList,
    });
  ...
  _createDataSource(list) {
    ...
    return dataSource.cloneWithRows(list.slice());
  }
  ...
  _toggleFullScreen(display: boolean) {
    ...
    this.props.onFullScreenToggle(display)
    Animated.timing(
    ...
    if (mediaList.length > 0) {
      ...
      fullScreenContainer = (
        <FullScreenContainer
          ...
          onActionButton={(data) => {onActionButton(data)}}
          onIndexUpdate={(index) => {onIndexUpdate(index)}}
    ...
    return (
      <View style={[styles.container, {
        ...
        <TopBarComponent
          ...
          title={!isFullScreen ? `${title} (${mediaList.length})` : title}
          onBack={onBack}
        />
      </View>
    );
